 ...
 * Exposer des services qui me sont utiles. (serveur web, serveur de jeu, *VPN*, stockage de fichier, etc)   
...
Je prenais donc un vieil ordinateur récupéré et remis en état, le reformatais pour ensuite installer un os et faire mes tests. Mais au fur et à mesure du temps j'ai trouvé cette "logistique" fatigante :   
...
Aussi en cas de besoin d'exposer plusieurs services je n'avais pas envie de tout mettre sur un même serveur, sans rien diviser pour des raisons de sécurité, parmis d'autres.   
   
Seulement n'ayant pas un énorme *switch*, ni de câbles ethernet, ni beaucoup de prises électriques, ni une volonté de consommer beaucoup trop de courant; maintenir en fonctionnement beaucoup de matériel de récupération, ancien, pas forcément le plus fiable, n'était pas une option.   
   
Me retrouvant limité j'ai pensé à plusieurs solutions : la virtualisation et les containers.   
J'ai opté pour les deux solutions à des niveaux différents, comme nous le verrons par la suite.   
   
## Architecture

Donc voici les spécifications de la machine de récupération utilisée que j'avais sous la main :   
 * CPU : `Athlon II x2 255 3.1GHz` de `2010`   
 * RAM : `8GB`   
 * Carte Réseau : *gigabit ethernet* intégrée à la carte mère   
 * Stockage : HDD de `512GB`.   
   
Ce n'est pas très puissant, mais je n'avais pas besoin de beaucoup de puissance et surtout, c'est ce que j'avais sous la main.   
Mais si vous souhaitez le refaire, avec du meilleur matériel c'est parfait.   
Vérifiez toujours que votre cpu supporte la virtualisation et qu'elle est activée dans le *bios*.   
   
Voici donc comment j'ai architecturé mon projet :   
![schema illustration](../assets/images/homeserver/schema.png)   
   
Au départ je souhaitais me servir des services de ma box comme le [NAT](https://en.wikipedia.org/wiki/Network_address_translation), le firewall intégré, etc. J'ai donc configuré l'hôte pour que les machines virtuelles soient sur le réseau local, pour expérimenter et faire mes services privés (envies de domotique dans le futur) accessibles en *LAN*.

L'idée était donc de me servir du NAT de la box pour les rares services exposés.   
   
Sauf que ma box actuelle est si mauvaise qu'elle arrive pas à enregistrer les paramètres nat entre autres. Heureusement la fonctionnalité [DMZ](https://en.wikipedia.org/wiki/DMZ_(computing)) "marche", enfin comme le font certains [ISP](https://en.wikipedia.org/wiki/Internet_service_provider) avec leur box, pas une vraie *DMZ* avec un second sous réseau, ni même un vlan, mais juste refiler tout le trafic à une machine du réseau locale.   
Une vraie passoire si la machine exposée n'a pas un firewall bien configuré.   
...
J'ai donc eu besoin d'un hyperviseur. Étant assez porté sur l'OpenSource j'ai écarté [VMWare](https://www.vmware.com/fr/products/workstation-pro.html). Par ailleurs je ne suis même pas sûr que mon matériel l'aurait supporté. J'ai fini par chercher une solution faite sur une distribution linux.
...
Pour la virtualisation, j'ai donc choisi [qemu-kvm](https://www.qemu.org/) avec [libvirt](https://www.libvirt.org/index.html). C'est performant, c'est très utilisé, parmi les mieux intégrés à linux, et cette solution étant conseillée et détaillée dans la documentation [redhat](https://www.redhat.com/en), c'est un plus.   
...
J'ai également installé ovmf pour pouvoir faire des vm en uefi.
...
Ensuite nous allons configurer le réseau de l'hôte.  Pour que les machines virtuelles soient sur le *LAN*, nous allons créer un *bridge* virtuel.  Pour ça il y a deux méthodes, la méthode avec le xml de libvirt, et la méthode externe. Pour ma part, j'ai choisis la méthode externe que voici. Avec [networkmanager](https://doc.ubuntu-fr.org/network-manager) et en particulier, *nmcli*, j'ai donc créé un bridge virtuel :

```sh   
nmcli con add ifname br0 type bridge con-name br0
```

Et ensuite pour que mon hôte puisse après coup être considéré comme connecté au bridge pour pouvoir avoir accès à internet par la même interface, je passe l'interface hôte, ici eth0, en *slave* de ce bridge :

```sh   
nmcli con add type bridge-slave ifname eth0 master br0
```

Enfin, avoir une addresse ip statique pour la connection à l'hôte est quand même très pratique, du coup, pour l'*ipv4* :
...
Vous pouvez faire la même chose pour l'*ipv6*, pour ma part, ma box ne supporte pas l'*ipv6*. Enfin nous pouvons activer cette interface :
...
N'ayant besoin que du *ssh*, la zone publique me convenait, et j'ai donc choisi public comme zone par défaut. La zone *libvirt* convient aussi.
...
Nous allons maintenant monter la machine virtuelle qui va me servir de firewall. J'ai choisi *alpine linux* pour sa légèreté, sa sécurité et sa faible consommation de ressources. J'ai donc téléchargé la dernière iso sur le site officiel de alpine linux sur le disque du serveur, et avec
...
Cette commande est pour lancer la procédure d'installation dans un terminal, sans interface graphique, il est aussi possible de passer par du vnc, pour des distributions avec installateurs graphique. D'autres détails peuvent varier selon l'os à virtualiser, et la façon de l'installer.   
...
Ensuite, j'ai lancé la procédure d'installation de alpine linux avec un serveur openssh et une ip fixe, ici `192.168.0.101`, puis je me suis connecté en ssh, car c'est plus pratique. La configuration du serveur ssh est hors de la portée de cet article.   
...
La suite de l'article porte sur l'architecture pour des services publiques, créer des machine virtuelles **suffisent en LAN pour des services privés**. La suite de l'article est fait pour l'exposition de services externes. Les commandes suivantes se feront sur la machine virtuelle qui sera utilisée comme firewall.   
...
Pour pouvoir le configurer il faut lancer le service. Ces étapes sont relativements "critiques", dans le sens où il y a un risque de s'enfermer dehors.   
...
Tant que vous sauvegardez pas, relancer le service suffit à revenir aux paramètres précédents.   
Evidemment il est possible d'enlever des règles facilement, mais cet article n'a pas vocation d'être une documentation de nftables.   
Quand vous sauvegardez soyez bien sûr que votre configuration ne vous enferme pas dehors.   
Pour sauvegarder :
...
### Ip forwarding   
Pour autoriser le forwarding, il faut le permettre au niveau du kernel, c'est possible via des réglages avec *sysctl*.   
 ...
 * Autre possibilité, configurer le bridge virtuel sur l'hôte pour faire un *vlan* (aucune idée pour le moment de comment m'y prendre), pour que les vm exposées n'aie le droit que de communiquer entre elle et avec le monde extérieur mais pas au reste du *LAN*.   
